...
Function Get-Mailboxes {
  ...
  process {
    ...
    Get-EXOMailbox -ResultSize 500 -RecipientTypeDetails $mailboxTypes -Properties IssueWarningQuota, ProhibitSendReceiveQuota, ArchiveQuota, ArchiveWarningQuota, ArchiveDatabase | 
...
Write-Host "Parameters finished" -ForegroundColor Yellow

# Connect to Exchange Online
Connect-EXO

Write-Host "Exchange Online is connected" -ForegroundColor Yellow

# Get mailbox status
Get-MailboxStats | Export-CSV -Path $path -NoTypeInformation -Encoding UTF8

Write-Host "Retrieved Mailbox" -ForegroundColor Yellow
